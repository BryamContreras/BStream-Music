...
Future<void> main(List<String> args) async {
  ...
  try {
    ...
    for (var i = 0; i < orderedClients.length; i++) {
      ...
      print('# [ATTEMPT] $videoId | $clientName | watchPage=$requireWatchPage | solver=true | chainPos=$i');
      ...
      try {
        ...
        if (selectedStream != null) {
          ...
          print('');
          print('# RESULT: success | client=$clientName | elapsed=${totalStopwatch.elapsedMilliseconds}ms | streams=${audioOnly.length} | codec=${selectedStream.audioCodec} | bitrate=${selectedStream.bitrate.bitsPerSecond}');
          break;
        } else {
          totalStopwatch.stop();
          print('# [NO_AUDIO] $clientName returned ${manifest.streams.length} streams but no audio in ${attemptStopwatch.elapsedMilliseconds}ms');
          print('');
          print('# RESULT: no-audio | client=$clientName | elapsed=${totalStopwatch.elapsedMilliseconds}ms | streams=${manifest.streams.length} | audio=${audioOnly.length}');
        }
        ...
        if (!isLastClient) {
          ...
          totalStopwatch.stop();
          print('');
          print('# RESULT: rate-limited | elapsed=${totalStopwatch.elapsedMilliseconds}ms | clients-attempted=${i + 1}');
        }
